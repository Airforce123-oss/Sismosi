...
const currentMonthYear = computed(() => {
    ...
    const futureMonthIndex = (currentMonth + 4) % 12; // Menambahkan 4 bulan ke bulan sekarang
    const futureYear = currentYear + Math.floor((currentMonth + 4) / 12); // Menambah tahun jika bulan melewati Desember

    const monthName = months[futureMonthIndex]; // Mendapatkan nama bulan ke-4
    return `${monthName} ${futureYear}`; // Format "Bulan YYYY"
});
        ...
        if (
            ...
            studentId.value = [
                ...new Set([
                    ...
                    ), // Menyaring hanya ID yang valid
                    ...nextPageData.filter(
                        (student) => typeof student === "number"
                    ), // Menyaring data halaman berikutnya
                ]),
            ];
